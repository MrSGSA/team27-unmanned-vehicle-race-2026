...
extent={{-10,-10},{10,10}})),__MWORKS(BlockSystem(Instance(u(Type(ref="double") ,Dimension=1) ,y(Type(ref="double") ,Dimension=1)),SampleTime(group="D1")=0.02)));
  SysplorerEmbeddedCoder.Discontinuities.Saturation saturation(zeroCross=true,upLimit=0.4,lowLimit=-0.4) 
    annotation (Placement(transformation(origin={136,-20}, 
...
grid={2,2})));
    ...
    State state 
      annotation (Placement(transformation(origin={-480.5,156}, 
extent={{-149.5,-68.5},{149.5,68.5}})),__MWORKS(BlockSystem(StateMachine(priority=1))));
    ...
    SysplorerEmbeddedCoder.Types.Auto k_rep annotation (__MWORKS(internalShare = true,BlockSystem(Type(inherit=InheritType.none,ref="double"),Dimension(dimensionType=DimensionType.none)=1)));
    SysplorerEmbeddedCoder.Types.Auto k_steer annotation (__MWORKS(internalShare = true,BlockSystem(Type(inherit=InheritType.none,ref="double"),Dimension(dimensionType=DimensionType.none)=1)));
    SysplorerEmbeddedCoder.Types.Auto epsilon annotation (__MWORKS(internalShare = true,BlockSystem(Type(inherit=InheritType.none,ref="double"),Dimension(dimensionType=DimensionType.none)=1)));
    ...
    SysplorerEmbeddedCoder.Types.Auto speed_min annotation (__MWORKS(internalShare = true,BlockSystem(Type(inherit=InheritType.none,ref="double"),Dimension(dimensionType=DimensionType.none)=1)));
    ...
    SysplorerEmbeddedCoder.Types.Auto speed_factor annotation (__MWORKS(internalShare = true,BlockSystem(Type(inherit=InheritType.none,ref="double"),Dimension(dimensionType=DimensionType.none)=1)));
    SysplorerEmbeddedCoder.Types.Auto MAX_SPEED(start=-50) annotation (__MWORKS(internalShare = true,BlockSystem(Type(inherit=InheritType.none,ref="double"),Dimension(dimensionType=DimensionType.none)=1)));
    SysplorerEmbeddedCoder.Types.Auto MAX_REVERSE(start=10) annotation (__MWORKS(internalShare = true,BlockSystem(Type(inherit=InheritType.none,ref="double"),Dimension(dimensionType=DimensionType.none)=1)));
    SysplorerEmbeddedCoder.Types.Auto SAFE_DISTANCE(start=30) annotation (__MWORKS(internalShare = true,BlockSystem(Type(inherit=InheritType.none,ref="double"),Dimension(dimensionType=DimensionType.none)=1)));
    SysplorerEmbeddedCoder.Types.Auto REVERSE_DISTANCE(start=20) annotation (__MWORKS(internalShare = true,BlockSystem(Type(inherit=InheritType.none,ref="double"),Dimension(dimensionType=DimensionType.none)=1)));
    State5 state4 
      annotation (Placement(transformation(origin={-824,156}, 
  extent={{-119,-76},{119,76}})),__MWORKS(BlockSystem(StateMachine(priority=2))));
    block State "状态"
      annotation (__MWORKS(BlockSystem(blockKind=BlockKind.state,SampleTime(auto = true),State,StateMachine),showDiagram = true,sourceModel=SysplorerEmbeddedCoder.StateMachine.State,independentInstance=true,hide=true), defaultComponentName = "state", 
       ...
       Diagram(coordinateSystem(extent={{-149.5,-68.5},{149.5,68.5}}, 
initialScale=1, 
grid={2,2}),graphics = {State(origin={0,0}, 
stateTitle=Text(origin={0,0}, 
lineColor={74,84,154}, 
extent={{-147.5,60.5},{147.5,68.5}}, 
textString="%name", 
fontSize=16, 
...
verticalAlignment=TextAlignment.Top))}),Protection(access=Access.packageDuplicate));
    algorithm
    annotation(__MWORKS(BlockSystem(StateMachine(actionKind = ActionKind.entry))));  k_rep := 20;
      k_steer := 0.2;
      epsilon := 0.5;
      speed_base := -20;
      front_threshold := 30;
    ...
    annotation(__MWORKS(BlockSystem(StateMachine(actionKind = ActionKind.during))));  F_front := -k_rep / ((df + epsilon) * (df + epsilon));
      ...
      steer := -atan2(0.707 * (F_right * 1.0 - F_left * 1.0), F_front + 0.707 * (F_left + F_right)) * k_steer;
      speed := -60 * min(df / 40, 1.0);
      if dl > 45 and dr > 45 then
        steer := 0.1;
      end if;
    end State;
    block State5 "状态"
      annotation (__MWORKS(BlockSystem(blockKind=BlockKind.state,SampleTime(auto = true),State),showDiagram = true,sourceModel=SysplorerEmbeddedCoder.StateMachine.State,independentInstance=true,hide=true), defaultComponentName = "state", 
       Icon(coordinateSystem(extent={{-100, -100}, {100, 100}}, 
         ...
         horizontalAlignment = TextAlignment.Left, 
         verticalAlignment = TextAlignment.Top)}), 
       Diagram(coordinateSystem(extent={{-119,-76},{119,76}}, 
  initialScale=1, 
  grid={2,2}),graphics = {State(origin={0,0}, 
  stateTitle=Text(origin={0,0}, 
  lineColor={74,84,154}, 
  extent={{-117,68},{117,76}}, 
  textString="%name", 
  fontSize=16, 
  textStyle={TextStyle.Bold}, 
  textColor={74,84,154}, 
  ...
  verticalAlignment=TextAlignment.Top))}),Protection(access=Access.packageDuplicate));
    ...
    annotation(Line(origin={-479.988,238.808}, 
    points={{0.401261,14.1471},{-0.240756,-14.3076}}, 
    color={113,119,170}, 
    ...
    transition(state, state4, df<18 or dl <15 or dr <15, reset = false) 
    annotation(Line(origin={-667.5,211.462}, 
    points={{37.5,0.03143},{-37.5,-0.03143}}, 
    color={113,119,170}, 
    smooth=Smooth.Bezier),Text(origin={-9.28546,7.53172}, 
    lineColor={74,84,154}, 
    extent={{0,-3},{0,3}}, 
    textString="%condition", 
    fontSize=10, 
    textStyle={TextStyle.Bold}, 
    textColor={74,84,154}, 
    horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = false))), displayText = "[df<18 or dl <15 or dr <15]");
    transition(state4, state, df>35, reset = false) 
    annotation(Line(origin={-667.5,162.331}, 
    points={{-37.5,2.25},{37.5,-2.25}}, 
    color={113,119,170}, 
    smooth=Smooth.Bezier),Text(origin={-5.622,6.42514}, 
    lineColor={74,84,154}, 
    extent={{0,-3},{0,3}}, 
    textString="%condition", 
    fontSize=10, 
    textStyle={TextStyle.Bold}, 
    textColor={74,84,154}, 
    horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = false))), displayText = "[df>35]");
    end Chart;
...
points={{19.2,38},{15,38},{15,60},{51,60},{51,86},{46.8,86}}, 
...
points={{-19.2,-23.3},{-19.2,3.25},{-5,3.25},{-5,29.8},{37.2,29.8}}, 
color={0,0,0}));
  connect(saturation.y, steer) 
  annotation(Line(origin={159,-45}, 
points={{-11.2,25},{7,25},{7,-13}}, 
color={0,0,0}));
  end APF;