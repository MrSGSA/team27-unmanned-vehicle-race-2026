model test3 "避障控制+差速（Sysblock）决赛版本"
...
grid={2,2})));
  ...
  extent={{-393,-238.75},{393,238.75}})),__MWORKS(BlockSystem(StateMachine(priority=1))));
    ...
    State1 state if false 
      annotation (Placement(transformation(origin={-860,-212}, 
  extent={{-216,-81},{216,81}})),__MWORKS(BlockSystem(StateMachine(priority=2))));
    block State "状态"
  ...
  verticalAlignment=TextAlignment.Top))}),Protection(access=Access.packageDuplicate));
      algorithm
      annotation(__MWORKS(BlockSystem(StateMachine(actionKind = ActionKind.entry or ActionKind.during))));  speed := -90;
        if dl < 35 then
          steer := 0.1;
        elseif dl > 40 then
          steer :=0;
...
extent={{-220.25,-141},{220.25,141}})),__MWORKS(BlockSystem(StateMachine)));
      State state1 
        annotation (Placement(transformation(origin={-302,55.25}, 
extent={{-40,-30},{40,30}})),__MWORKS(BlockSystem(StateMachine)));
  ...
  verticalAlignment=TextAlignment.Top))}),Protection(access=Access.packageDuplicate));
      ...
      equation
      transition(cruise, state, df <=30 or dr <= 30, reset = false) 
      ...
      textStyle={TextStyle.Bold}, 
      textColor={74,84,154}, 
      horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = false))), displayText = "[df <=30 or dr <= 30]");
      transition(state, cruise, df >45, reset = false) 
      annotation(Line(origin={-427.399,-144.19}, 
      points={{94.8992,-27.366},{-32.9865,5.58769},{-93.6008,7.63401}}, 
      color={113,119,170}, 
      ...
      transition(state, state1, df<10or dl<10 or dr<10, reset = false, priority = 2) 
      annotation(Line(origin={-439.601,-203.931}, 
      points={{107.100875,-14.321},{-60.710925,-32.069}}, 
      color={113,119,170}, 
      ...
      textStyle={TextStyle.Bold}, 
      textColor={74,84,154}, 
      horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = false))), displayText ="[df<10or dl<10 or dr<10]");
      transition(state1, state, after(1,sec), reset = false) 
      annotation(Line(origin={-409.25,-276.713}, 
      points={{-76.75,0},{76.75,0}}, 
      color={113,119,170}, 
      smooth=Smooth.Bezier),Text(origin={0,0}, 
      lineColor={74,84,154}, 
      extent={{0,-3},{0,3}}, 
      ...
      textStyle={TextStyle.Bold}, 
      textColor={74,84,154}, 
      horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = false))), displayText = "[after(1,sec)]");
      end State;
    block State1 "状态"
      annotation (__MWORKS(BlockSystem(blockKind=BlockKind.state,SampleTime(auto = true),State),showDiagram = true,sourceModel=SysplorerEmbeddedCoder.StateMachine.State,independentInstance=true,hide=true), defaultComponentName = "state", 
       Icon(coordinateSystem(extent={{-100, -100}, {100, 100}}, 
         ...
         horizontalAlignment = TextAlignment.Left, 
         verticalAlignment = TextAlignment.Top)}), 
       Diagram(coordinateSystem(extent={{-216,-81},{216,81}}, 
  initialScale=1, 
  grid={2,2}),graphics = {State(origin={0,0}, 
  stateTitle=Text(origin={0,0}, 
  lineColor={74,84,154}, 
  extent={{-214,73},{214,81}}, 
  textString="%name", 
  fontSize=16, 
  textStyle={TextStyle.Bold}, 
  textColor={74,84,154}, 
      ...
      block State5 "状态"
        annotation (__MWORKS(BlockSystem(blockKind=BlockKind.state,SampleTime(auto = true),State),showDiagram = true,sourceModel=SysplorerEmbeddedCoder.StateMachine.State,independentInstance=true,hide=true), defaultComponentName = "state", 
         ...
         Diagram(coordinateSystem(extent={{-40, -30}, {40, 30}}, initialScale=1, grid={2, 2}), graphics = {State(origin = {0.0, 0.0}, 
              ...
              verticalAlignment = TextAlignment.Top))}),Protection(access=Access.packageDuplicate));
      algorithm
      annotation(__MWORKS(BlockSystem(StateMachine(actionKind = ActionKind.entry or ActionKind.during))));  u4 := -0.6;
        fr := dfr;
        speed := 0;
      end State5;
      State5 state1 if false 
      annotation(Placement(transformation(origin={12,-3}, 
  extent={{-40,-30},{40,30}})),__MWORKS(BlockSystem(StateMachine)));
      State6 state2 if false 
      annotation(Placement(transformation(origin={148,3}, 
  extent={{-40,-30},{40,30}})),__MWORKS(BlockSystem(StateMachine)));
      State state if false 
        annotation (Placement(transformation(origin={-130,1}, 
  extent={{-40,-30},{40,30}})),__MWORKS(BlockSystem(StateMachine)));
      block State6 "状态"
        annotation (__MWORKS(BlockSystem(blockKind=BlockKind.state,SampleTime(auto = true),State),showDiagram = true,sourceModel=SysplorerEmbeddedCoder.StateMachine.State,independentInstance=true,hide=true), defaultComponentName = "state", 
         Icon(coordinateSystem(extent={{-100, -100}, {100, 100}}, 
      ...
      block State "状态"
        annotation (__MWORKS(BlockSystem(blockKind=BlockKind.state,SampleTime(auto = true),State),showDiagram = true,sourceModel=SysplorerEmbeddedCoder.StateMachine.State,independentInstance=true,hide=true), defaultComponentName = "state", 
         ...
         Diagram(coordinateSystem(extent={{-40, -30}, {40, 30}}, initialScale=1, grid={2, 2}), graphics = {State(origin = {0.0, 0.0}, 
              ...
              verticalAlignment = TextAlignment.Top))}),Protection(access=Access.packageDuplicate));
      algorithm
      annotation(__MWORKS(BlockSystem(StateMachine(actionKind = ActionKind.entry or ActionKind.during))));  u4 := 0;
      end State;
    equation
      transition(state1, state2, after(3, sec), reset = false) 
      annotation(Line(origin={-780,-211.4328}, 
      points={{-28,2.84217e-14},{28,-2.84217e-14}}, 
      color={113,119,170}, 
      smooth=Smooth.Bezier),Text(origin={-12.6715,8.63963}, 
      lineColor={74,84,154}, 
      extent={{0,-3},{0,3}}, 
      textString="%condition", 
      fontSize=10, 
      textStyle={TextStyle.Bold}, 
      textColor={74,84,154}, 
      horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = true))), displayText = "[after(3, sec)]");
      transition(state, state1, flag1, reset = false) 
      annotation(Line(origin={-919,-217.978}, 
      points={{-31,1.10488},{31,-1.10488}}, 
      color={113,119,170}, 
      smooth=Smooth.Bezier),Text(origin={0,0}, 
      lineColor={74,84,154}, 
      extent={{0,-3},{0,3}}, 
      ...
      fontSize=10, 
      textStyle={TextStyle.Bold}, 
      textColor={74,84,154}), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = true))), displayText = "[flag1]");
      transition(state2, state, not flag1, reset = false) 
      annotation(Line(origin={-851.923,-165.827}, 
      points={{110.945,-13.173},{11.5437,0.205854},{-111.099,-15.173}}, 
      color={113,119,170}, 
      smooth=Smooth.Bezier),Text(origin={1.78969,8.35187}, 
      lineColor={74,84,154}, 
      extent={{0,-3},{0,3}}, 
      textString="%condition", 
      fontSize=10, 
      textStyle={TextStyle.Bold}, 
      textColor={74,84,154}, 
      horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = true))), displayText = "[not flag1]");
    algorithm
    annotation(__MWORKS(BlockSystem(StateMachine(actionKind = ActionKind.entry or ActionKind.during))));  if flag1 and fr > fl then
        steer := 0.3;
      elseif flag1 and fl > fr then
        steer := -0.2;
      end if;
    end State1;
    equation
    ...
    initialState(state.state, true, 1) 
    annotation(Line(origin={-994.543,-171}, 
    points={{0,10},{0,-10}}, 
    color={113,119,170}, 
    smooth=Smooth.Bezier),Text(origin={0,0}, 
    lineColor={74,84,154}, 
