model ObsAvoidController_Sysblock7g "避障控制+差速（Sysblock）决赛版本"
...
extent={{-220.25,-141},{220.25,141}})),__MWORKS(BlockSystem(StateMachine)));
      State2 state1 
        annotation (Placement(transformation(origin={-282,28.25}, 
extent={{-60,-57},{60,57}})),__MWORKS(BlockSystem(StateMachine)));
      block State1 "状态"
        annotation (__MWORKS(BlockSystem(blockKind=BlockKind.state,SampleTime(auto = true),State),showDiagram = true,sourceModel=SysplorerEmbeddedCoder.StateMachine.State,independentInstance=true,hide=true), defaultComponentName = "state", 
         Icon(coordinateSystem(extent={{-100, -100}, {100, 100}}, 
           grid={2, 2}), graphics={Rectangle(origin = {0, 0}, 
      ...
      block State2 "状态"
        annotation (__MWORKS(BlockSystem(blockKind=BlockKind.state,SampleTime(auto = true),State),showDiagram = true,sourceModel=SysplorerEmbeddedCoder.StateMachine.State,independentInstance=true,hide=true), defaultComponentName = "state", 
         Icon(coordinateSystem(extent={{-100, -100}, {100, 100}}, 
           ...
           horizontalAlignment = TextAlignment.Left, 
           verticalAlignment = TextAlignment.Top)}), 
         Diagram(coordinateSystem(extent={{-60,-57},{60,57}}, 
initialScale=1, 
grid={2,2}),graphics = {State(origin={0,0}, 
stateTitle=Text(origin={0,0}, 
lineColor={74,84,154}, 
...
verticalAlignment=TextAlignment.Top))}),Protection(access=Access.packageDuplicate));
      ...
      annotation(__MWORKS(BlockSystem(StateMachine(actionKind = ActionKind.entry or ActionKind.during))));  speed := 30;
        if dr < 5 and dl < 5 then
          steer := 0;
          speed := -10;
        elseif abs(dr - dl) < 5 then
          steer := 0.2;
          speed := max(30 * (1.0 - df / 20) + 2, 30);
        elseif dr > dl then
          speed := max(30 * (1.0 - df / 20) + 2, 30);
          steer := -0.3;
        elseif dr < dl then
    ...
    equation
      transition(cruise, state, df <=20 or dr <= 20 or dl<20, reset = false) 
      annotation(Line(origin={-426.75,-135.166}, 
points={{-94.25,15.8399},{94.25,-15.8399}}, 
color={113,119,170}, 
smooth=Smooth.Bezier),Text(origin={-14.7921,11.8341}, 
lineColor={74,84,154}, 
extent={{0,-3},{0,3}}, 
...
textStyle={TextStyle.Bold}, 
textColor={74,84,154}, 
horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition=true,comment=false))), displayText ="[df <=20 or dr <= 20 or dl<20]");
      transition(state, cruise, df >45 and dr >30, reset = false) 
      annotation(Line(origin={-428.2,-154.016}, 
      points={{95.7,-17.5403},{-11.983,2.29424},{-92.8,17.4596}}, 
      color={113,119,170}, 
      smooth=Smooth.Bezier),Text(origin={-21.8809,8.35323}, 
      lineColor={74,84,154}, 
      extent={{0,-3},{0,3}}, 
      textString="%condition", 
      fontSize=10, 
      textStyle={TextStyle.Bold}, 
      textColor={74,84,154}, 
      horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = false))), displayText ="[df >45 and dr >30]");
      transition(state1, state, dfr>45 and after(1,sec), reset = false) 
      ...
      textStyle={TextStyle.Bold}, 
      textColor={74,84,154}, 
      horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = false))), displayText ="[dfr>45 and after(1,sec)]");
      transition(state, state1, (df<10 or dfr <45 )and (dl<10 or dr<10), reset = false, priority = 2) 
      annotation(Line(origin={-439.601,-203.931}, 
      points={{107.101,-14.321},{-60.711,-32.069}}, 
      color={113,119,170}, 
      smooth=Smooth.Bezier),Text(origin={7.10037,4.49581}, 
      lineColor={74,84,154}, 
      extent={{0,-3},{0,3}}, 
      textString="%condition", 
      fontSize=10, 
      textStyle={TextStyle.Bold}, 
      textColor={74,84,154}, 
      horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition = true, comment = false))), displayText ="[(df<10 or dfr <45 )and (dl<10 or dr<10)]");
    end State;
    equation
...
color={0,0,0}));
  end ObsAvoidController_Sysblock7g;