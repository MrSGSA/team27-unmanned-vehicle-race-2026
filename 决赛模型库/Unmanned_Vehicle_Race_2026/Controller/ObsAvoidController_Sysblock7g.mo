...
extent={{-245.5,-90.5},{245.5,90.5}})),__MWORKS(BlockSystem(StateMachine(priority=3))));
  ...
  textColor={74,84,154}), displayText = "[true]",__MWORKS(BlockSystem(StateMachine(outerTransition=false))));
      transition(cruise, avoid, df <= mindis or dfl <= 5 or dfr <= 5, reset = false) 
  ...
  textStyle={TextStyle.Bold},
  textColor={74,84,154},
  horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition=true))), displayText ="[df <= mindis or dfl <= 5 or dfr <= 5]");
      transition(avoid, cruise, df > safeF and dfl > safeFLR and dfr > safeFLR, reset = false) 
...
textStyle={TextStyle.Bold},
textColor={74,84,154},
horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition=true))), displayText ="[df > safeF and dfl > safeFLR and dfr > safeFLR]");
    end State;
    block State1 "状态"
...
verticalAlignment=TextAlignment.Top))}),Protection(access=Access.packageDuplicate));
  ...
  extent={{-51,-20},{51,20}})),__MWORKS(BlockSystem(StateMachine)));
        block State1 "状态"
          ...
          points={{-40,22},{40,22}}))}),Protection(access=Access.packageDuplicate));
        algorithm
        annotation(__MWORKS(BlockSystem(StateMachine(actionKind = ActionKind.entry or ActionKind.during))));  steer := 0.7;
        end State1;
        block State2 "状态"
          annotation (__MWORKS(BlockSystem(blockKind=BlockKind.state,SampleTime(auto = true),State),showDiagram = true,sourceModel=SysplorerEmbeddedCoder.StateMachine.State,independentInstance=true,hide=true), defaultComponentName = "state",
           Icon(coordinateSystem(extent={{-100, -100}, {100, 100}},
...
textColor={74,84,154},
horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition=true))), displayText ="[dr - dl > mindis]");
        transition(ini_state, turn_right, dfl <= safeFLR, reset = false, priority = 3) 
        annotation(Line(origin={-147.064,-226.264},
points={{-118.936,8.17862},{55.3787,-71.7359}},
color={113,119,170},
...
textStyle={TextStyle.Bold},
textColor={74,84,154},
horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition=true))), displayText = "[dfl <= safeFLR]");
        transition(ini_state, turn_left, dfr <= safeFLR, reset = false, priority = 4) 
...
textStyle={TextStyle.Bold},
textColor={74,84,154},
horizontalAlignment=TextAlignment.Left), __MWORKS(BlockSystem(StateMachine(outerTransition=true))), displayText ="[dfr <= safeFLR]");
      end State;
...
extent={{-168,-92},{168,92}})),__MWORKS(BlockSystem(StateMachine)));
  ...
  extent={{-53,-20},{53,20}})),__MWORKS(BlockSystem(StateMachine)));
      ...
      equation
        transition(ini_state, turn_left, dfl <= safeFLR, reset = false) 
...
textStyle={TextStyle.Bold},
textColor={74,84,154}), displayText = "[true]",__MWORKS(BlockSystem(StateMachine(outerTransition=false))));
        transition(ini_state, turn_right, dfr <= safeFLR, reset = false, priority = 2) 
        annotation(Line(origin={150.0922,-245.509},
points={{-78.0922,8.9632},{34.5645,-52.4909}},
color={113,119,170},
smooth=Smooth.Bezier),Text(origin={0,0},
lineColor={74,84,154},
extent={{0,-3},{0,3}},
...
points={{-76.2,-22.6667},{-69,-22.6667},{-69,23},{77,23}},
color={0,0,0}));
